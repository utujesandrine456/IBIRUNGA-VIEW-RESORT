...
export function BookingFormSection() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.adults || !form.roomType) {
      setStatus("error");
      setMessage("Please select adults and a room type.");
      return;
    }
          ...
          <form className="space-y-4" onSubmit={handleSubmit} autoComplete="off">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field
                ...
                value={form.roomType}
                onChange={(v) => setForm({ ...form, roomType: v })}
                required
              />
            </div>
            <Field label="Special Requests" value={form.specialRequests} onChange={(v) => setForm({ ...form, specialRequests: v })} />
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Button className="mt-2 w-full rounded-md" type="submit" disabled={status === "loading"}>
